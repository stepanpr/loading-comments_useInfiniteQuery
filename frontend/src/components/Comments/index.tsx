...
/* структурирование комментариев */
  const nestComments = (commentList: IComment[]) => {
    ...
    commentList.forEach((comment: IComment) => commentMap[comment.id] = comment); //записываем все комментарии в commentMap
    ...
    return commentList.filter((comment: IComment) => { //всем ParentId присваиваем null
      return comment.parentId === null;
    });
  }
  ...
  const useComments = () => {
    ...
    const response = await fetch('/comments').then((response: any) => response.json())
...
function Comment({ comment }: ICommentObj) {
  ...
  return (
    <>
    { query.isError ? <div>Ошибка загрузки данных!</div> :
      (!query.isLoading ?
      (<div>
      ...
      </div>) : (<div>Загрузка данных...</div>) 
      )
    }
      <button className={'button is-normall is-primary is-light is-fullwidth' + (query.isLoading ? ' is-loading' : '')} onClick={handleClick}>Загрузить еще...</button>
    </>
  )
}
